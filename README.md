...
- **
          ...
          ：每个行星按照设定的公转速度绕太阳公转，模拟真实的太阳系运行情况。
...
- **
          ...
          ：太阳采用自定义的着色器材质，呈现出燃烧的火球效果，且火焰效果会动态变化。
...
- **
          ...
          ：包含页面标题、信息提示区域、控制按钮区域、显示星球名称的元素以及引入 Three.js 库和相关脚本。
...
- **
          ...
          ：通过 CSS 样式设置页面背景、信息提示和控制按钮的样式，确保界面美观易用。
...
- **
          ...
          ：创建 Three.js 的场景、相机和渲染器，并设置相机的初始位置和视角。
...
- **星球创建**
          ...
          ：定义 `createBody` 函数，用于创建太阳、行星和卫星，并设置它们的属性，如半径、颜色、距离、公转速度等。
...
-
          ...
          **
          ...
          ：使用 `requestAnimationFrame` 实现动画循环，更新行星的位置和太阳的火焰效果，并进行渲染。
...
-
          ...
          **
          ...
          ：监听鼠标移动事件，使用 `Raycaster` 检测鼠标与星球的交互，显示星球的中英文名称；监听控制按钮的点击事件，实现视角缩放、重置和轨道显示切换等功能。
